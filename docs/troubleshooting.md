...
- `yearly_top_species` — the top species over a rolling 12-month window, built from per-day `/daily-count` history. On a fresh install it starts from the first backfilled chunk, then fills in over the next few hours as the historical backfill walks back (~15 days/poll, throttled to be kind to the API). Rarity-derived sensors (`notable_species`, `rarest_species`) sharpen as that window fills.
- `rarest_species` — derived from the same per-day history; its 7-day window is available as soon as the backfill has fetched the last week (typically the first poll, which grabs ~15 days).
